...
echo "Number of enabled h2c channels = $h2cChannels"
...
echo "Number of enabled c2h channels = $c2hChannels"
# Report if the PCIe DMA core is memory mapped or streaming
if [ $isStreaming -eq 0 ]; then
  echo "The PCIe DMA core is memory mapped."
else
  echo "The PCIe DMA core is streaming."
fi
...
if [ $isStreaming -eq 0 ]; then
  ...
  if [ $channelPairs -gt 0 ]; then
    ...
    echo "No PCIe DMA stream channels were tested because no h2c/c2h pairs were found."
  fi
...
#echo "All tests in run_tests.sh passed."
exit 0
